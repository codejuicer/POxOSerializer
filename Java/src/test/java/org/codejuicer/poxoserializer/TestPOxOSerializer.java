...
import org.codejuicer.poxoserializer.POxOSerializer;
...
public class TestPOxOSerializer {
	...
	public void test1() throws POxOSerializerException {
		...
		retB = (PrimitiveClassesContainer) serializer.deserialize(serializer
				.serialize(classToTest));
		...
		assertEquals(retB.getfNotNull(), classToTest.getfNotNull(),
				0.0000000001);
		...
		assertEquals(retB.getNestedCollections().size(), classToTest
				.getNestedCollections().size());
		assertEquals(retB.getNestedCollections().get(0).size(), classToTest
				.getNestedCollections().get(0).size());
		assertEquals(retB.getNestedCollections().get(0).get("test").get(2),
				classToTest.getNestedCollections().get(0).get("test").get(2));
		assertEquals(retB.getNestedClass().get(0).getIndex(), classToTest
				.getNestedClass().get(0).getIndex());
		assertEquals(retB.getGenericValueMap().get("bool"), classToTest
				.getGenericValueMap().get("bool"));
		assertEquals(retB.getGenericValueMap().get("int"), classToTest
				.getGenericValueMap().get("int"));
		assertEquals(retB.getGenericValueMap().get("string"), classToTest
				.getGenericValueMap().get("string"));
		assertEquals(
				true,
				retB.getGenericValueMap().get("object") instanceof NestedObjectClass);
		assertEquals(
				((NestedObjectClass) retB.getGenericValueMap().get("object"))
						.getIndex(),
				((NestedObjectClass) classToTest.getGenericValueMap().get(
						"object")).getIndex());
	}
}
